...
public class UserProfileNotificationService
{
    ...
    public IDisposable SubscribeToUserProfileNotifications(string buyerId, Func<Task> callback)
    {
    ...
    public Task NotifyUserProfileChangedAsync(string buyerId)
    {
        lock (_subscriptionsLock)
        {
    ...
    private class Subscription(UserProfileNotificationService owner, string buyerId, Func<Task> callback) : IDisposable
    {
